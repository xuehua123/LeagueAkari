...
本文是范围基线，不代表所有条目已经实现。清单中的 `[ ]` 表示待开发、待验证或待完成；“本期暂不启用”表示能力已排入后续阶段，不属于永久红线；“授权与执行边界”表示实现必须遵守的条件，也不等于删除功能。只有达到对应阶段的完成定义，才可以将该阶段标记为完成。
...
| ------ | -------------- | ---------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------- |
| 第一期 | 小地图守望者   | 看见地图变化、预测迷雾风险、指导购买装备 | 本地短语音、迷雾预测、装备方案、字幕、状态提示 | 不使用 LLM，不使用麦克风；确定性观测、预测与推荐规则          | 全部开发；未完成外部审核时只由内部测试开关控制     |
| 第二期 | 小地图态势教练 | 看懂局势，解释风险，最多给两个选项       | 结构化宏观建议、证据卡片、关键时刻复盘         | 不使用生成式 AI；事实、场景、选项和语言均由本地确定性规则产生 | 全部开发；对外是否开启由地区、审核与质量开关控制   |
| 第三期 | 全模态主动教练 | 能问答、主动指挥、逐帧指导并长期训练     | PTT/唤醒词/常开麦、全画面与授权音频、多模式    | 受约束 LLM 排序已验证候选，不能创造当前事实或自动执行         | 全部开发；审核结果只切换公开可见性，不改变开发范围 |
...
1. **观测与推断分层**：当前屏幕中的小地图像素是空间观测的核心依据；系统必须在此基础上结合 last-seen、移动速度、可达路径、游戏时间、已发生事件和合法对局状态，生成不可见敌人的概率区域、可能路线与意图预测。观测和预测必须分层展示，预测不能伪装成直接看见的事实。
2. **当前事实优先**：实时观察高于历史倾向；历史数据不得覆盖当前可见证据。
3. **不确定就沉默**：某项能力的必要传感器不可用、置信度不足、证据过期或数据冲突时，只停止依赖该证据的输出；例如小地图失效关闭宏观地图判断，但不自动关闭仍有完整画面证据的近身微操分析。
4. **描述风险并给推荐**：产品可以排序并突出当前首选，同时保留依据、适用条件和至少一个可行备选；不得把建议伪装成必然正确的强制命令，也不得自动执行。即使信息来自当前可见画面，仍需逐功能审核对外启用条件。
...
允许说“当前更推荐先控龙，备选是换上塔”，但不得使用“保证、必须、唯一正确”等无条件措辞，也不得替用户执行动作。
...
- [ ] 基于英雄、位置、金币、已有装备、游戏版本和局势生成当前对局的购买装备指导，并给出备选购买路径；
- [ ] 基于当前可见小地图、last-seen、移动趋势、地图路径和已发生事件，推断不可见敌人的概率位置、可能路线、游走/埋伏/包夹等意图与战争迷雾风险。
- [ ] 战斗微操、技能连招、补刀、走位、技能加点、对线和团战站位指导；
- [ ] 基于允许接口或屏幕可见行为的技能、召唤师技能、大招、野怪、眼位和资源计时；
- [ ] 由用户触发或经单独授权的 Ping、快捷喊话、自动聊天摘要和团队沟通建议；
- [ ] 经单独授权的唤醒词、常开麦克风、队伍语音/系统声音分析、情绪与沟通状态辅助；
- [ ] 受约束的完整画面多模态分析、主动战术指挥、其他地图/模式和跨地区合法数据增强。

#### 红色：三期均永久排除

永久红线只描述**未经授权的技术行为、数据行为或规避行为**，不再把某个教练功能本身、质量要求、营销措辞或“当前尚未获批”列成红线。未来如果 Riot/腾讯提供正式 SDK、自动化接口或书面授权，必须通过 ADR 将对应路径移出红线后再开发。

- [ ] 未经 Riot/腾讯正式 SDK、公开接口或书面授权，读取或写入游戏进程内存；
- [ ] 未经授权进行 DLL 注入、游戏 Hook、驱动注入、封包拦截、协议篡改或流量解密；
- [ ] 未经正式自动化接口或书面授权，通过键鼠模拟、输入注入、脚本或机器人替玩家执行移动、施法、购买、点击等游戏操作；
- [ ] 绕过、隐藏或规避 Vanguard/反作弊检测；
- [ ] 要求用户关闭安全软件、反作弊或系统保护；
- [ ] 未经相关人员授权采集、保存、上传或分析其语音、身份、私人通信，或克隆其真人音色；
- [ ] 窃取、复用、转售或绕过授权获取账号凭据、Provider Key、受保护接口 token 与非公开数据。

以下内容是**质量与产品诚信门槛，不是研发红线**：预测必须标注不确定性；冷却和计时必须标注来源；不得把预测伪装成直接观测；不得承诺保证上分、保证胜率或零误报。违反时阻止正式输出或发布，但相关功能代码、离线测试和修复工作继续保留。
...
flowchart LR
  ...
  VOICE[CoachVoiceMain - Phase 3<br/>PTT / wake / continuous / authorized audio / ASR]
...
| `minimap-observer-main` | 第一期   | 采集、ROI 标定、当前可见图标检测、追踪、last-seen 与运动特征               | 不直接生成购买或战术建议                          |
| `live-coach-main`       | 第一期   | 会话、事实融合、迷雾推断、装备推荐、确定性规则、提示调度、证据链、复盘事件 | 不直接管理浏览器窗口，不把 LLM 当事实源           |
| `live-coach-renderer`   | 第一期   | 同步精简 DTO、设置、诊断、用户反馈和页面状态                               | 不接收高频原始帧，不运行高频检测                  |
| `coach-voice-main`      | 第三期   | PTT、唤醒词、持续收音、授权音频源、ASR、取消、设备与云端/本地语音编排      | 不独立创造对局事实，不绕过 `live-coach-main` 校验 |
...
interface RecordedObservation<T> extends EvidenceBase<'minimap-replay' | 'replay-sidecar', T> {
...
interface HistoricalEvidence<T> extends EvidenceBase<'lcu-history' | 'sgp-history', T> {
...
| -------------------- | ------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------ |
| 刚采集的小地图像素   | 当前          | 当前画面中确实可见的图标、区域、数量和短时几何趋势                                     | 不能单独证明迷雾内真实位置、确定意图、确定路线或未显示冷却；可以作为概率推断证据 | 失败时观测为 `unknown`，依赖该观测的预测同步失效 |
...
| 第三期完整画面       | 当前          | 屏幕中实际显示的血条、技能表现、角色相对位置、动作和战斗状态                           | 迷雾内事实、未显示冷却、跨地图路线；不能替代小地图宏观位置                       | 失败时只关闭逐帧微操和依赖它的战斗评估           |
| 第三期授权音频       | 当前/对话     | 已识别的语音内容、声学事件、沟通强度和可验证协同意图                                   | 未说出的想法、真实身份或游戏事实；游戏结论需与视觉/状态证据交叉验证              | 撤权或来源失败时立即清空片段并关闭依赖能力       |
| 第三期 LLM           | 表达层        | 表达本地已验证的观测、迷雾预测、装备方案和候选集合；排序首选、说明备选、适用条件或拒答 | 自行创造当前观测、无证据预测、候选集合外动作或自动执行                           | 失败回退本地模板                                 |
...
- [ ] **P1-065** 敌人消失后生成 last-seen 记录并进入迷雾推断；输出必须从“当前可见”切换为“预测”，不得把预测位置标成当前直接观测；
- [ ] **P1-066** 识别多个当前可见敌方图标在同一区域聚集；
- [ ] **P1-067 [必做]** 识别多个当前可见图标从不同方向朝玩家或某区域接近的几何趋势，并在达到独立阈值时输出“可能包夹/游走”等意图预测；
...
- [ ] **P1-076 [必做]** 为不可见敌人维护基于 last-seen、英雄移动能力、地图可达图和经过时间的概率区域；
- [ ] **P1-077 [必做]** 输出最可能区域、候选路线、预计到达时间范围和置信度，不把单点坐标当成确定事实；
- [ ] **P1-078 [必做]** 推断游走、回城、埋伏、包夹、资源集结和换线等意图，每项必须携带支持证据与反证条件；
- [ ] **P1-079 [必做]** 新可见观察、死亡、回城确认、目标事件或时间超限时，立即更新或撤销相关迷雾预测。

### 7.7 第一期教练事件

第一期播报高置信、短时有效的直接观测、概率预测和装备购买指导；播报必须明确使用“当前看到”“可能”“预计”等不同语义。
...
- [ ] **P1-088** 提示可以包含直接观测、概率预测、装备购买指导、一个当前首选和一个备选，但必须标明依据与不确定性，不生成无条件强制命令；
- [ ] **P1-089** 敌人图标消失后启动 Missing、可能路线与抓人意图推断；只有超过阈值才正式提醒，并同时播报依据、置信等级和有效时间；
- [ ] **P1-090** 规则引擎完全确定性，可由同一回放稳定复现相同输出；
- [ ] **P1-091** 每条提示保存触发规则版本和证据 ID，支持赛后审计；
- [ ] **P1-092** 没有满足条件的事件时保持安静，不为维持存在感而讲话。
- [ ] **P1-093 [必做]** 读取本人当前金币、已有装备、英雄、位置、等级、游戏时间和当前版本装备数据；
- [ ] **P1-094 [必做]** 生成当前回城窗口的购买装备指导，至少提供核心装、组件、鞋子、消耗品和真眼等合法候选；
- [ ] **P1-095 [必做]** 同时给出推荐购买顺序、金币差额、可替代组件和购买后的剩余金币；
- [ ] **P1-096 [必做]** 推荐解释必须引用版本、英雄、已有装备、金币与局势证据，不使用已删除或当前模式不可用的装备；
- [ ] **P1-097 [必做]** 用户可选择常用出装、系统推荐、对局自适应或自定义装备方案；
- [ ] **P1-098 [必做]** 装备指导只提供信息和选项，不调用商店接口、不发送输入、不自动购买；
- [ ] **P1-099 [必做]** 回放和测试夹具覆盖不同英雄、位置、金币断点、装备栏已满、重复唯一装备和补丁变化。
...
- [ ] **P1-113 [必做]** 提供技能加点、基础连招、补刀、控线、走位、对线和团战站位指导；
- [ ] **P1-114 [必做]** 记录屏幕可见或允许接口提供的技能、召唤师技能、大招、野怪、眼位和资源事件；
- [ ] **P1-115 [必做]** 输出计时来源、时间范围和可信度；没有观察或允许来源时显示未知；
- [ ] **P1-116 [必做]** 根据教练事件推荐 Ping 类型、位置和简短喊话；
- [ ] **P1-117 [必做]** 支持用户通过快捷键确认发送 Ping/喊话，并可编辑模板和限制频率；
- [ ] **P1-118 [必做]** 正式允许接口与独立授权同时成立时支持自动发送 Ping/聊天；公开状态由独立 capability 控制；
- [ ] **P1-119** 不使用键鼠模拟或输入注入发送通信，不把自动通信扩展为移动、施法、购买或点击控制。
...
### 7.12 第一期暂不启用与执行边界（不属于永久红线）

- [ ] **P1-X01** 不使用 LLM、云端生成式 AI 或自由文本提示；
- [ ] **P1-X02** 不申请麦克风，不做问答、唤醒词或持续监听；
- [ ] **P1-X03** 实时微操、连招、走位、技能释放和补刀指导排入第三期；一期完成装备购买、技能加点和基础对线指导，但不执行游戏输入；
- [ ] **P1-X04** 不把不可见敌人、战争迷雾、路线或意图的推断伪装成直接观测事实；预测能力本身属于必做；
- [ ] **P1-X05** 基础可见技能/资源计时进入一期；完整敌方冷却推断排入第二期并使用独立外部开关；
- [ ] **P1-X06** 一期实现用户触发的 Ping/快捷喊话建议；自动发送仅能走允许接口和独立授权，不能通过输入模拟控制游戏；
- [ ] **P1-X07** 4K、超宽屏、HDR、色盲模式和极端 UI 缩放排入第二期；
- [ ] **P1-X08** 极地大乱斗、斗魂竞技场和其他地图排入第三期；
...
第二期在第一期可靠事实层之上增加宏观局势理解。每条提示仍必须基于可见证据和确定性状态机，输出“观察 + 影响 + 当前首选 + 备选”，建议可以排序但不能伪装成必然正确或自动执行。
...
- [ ] **P2-011** 为最近一次可见位置建立带置信度衰减的 last-seen 记录，并作为迷雾概率模型的起点；
- [ ] **P2-012** 基于 last-seen、英雄速度、位移能力、地图路径和经过时间生成动态概率区域；界面必须区分最后可见点、预测区域和当前直接观测；
- [ ] **P2-013** 计算可见图标短时间移动向量、速度和方向趋势；
- [ ] **P2-014** 识别多名单位的聚集、分散、交汇和多方向接近，并生成游走、埋伏、包夹、资源集结等意图概率；
- [ ] **P2-015** 同时计算当前可见人数和包含迷雾概率的风险人数区间，明确区分观测值与估计值；
...
- [ ] **P2-085** last-seen 与路线概率参与当前风险结论；随时间衰减，超过模型有效期后自动撤销而不是长期保留；
...
- [ ] **P2-096 [必做]** 根据英雄、对线、兵线、技能状态和可见战斗证据提供条件式补刀、连招、换血、走位和团战站位指导；
- [ ] **P2-097 [必做]** 对可见施放或允许接口证明的普通技能、召唤师技能和大招建立冷却估计，展示来源、时间范围、修正条件和置信度；
- [ ] **P2-098 [必做]** 基于宏观状态机生成 Ping/喊话/聊天方案，支持用户确认与获准自动发送两种路径；
- [ ] **P2-099** 所有自动通信独立限频、可审计、可立即停止，并受地区、接口和审核 capability 控制。
...
### 8.10 第二期暂不启用与执行边界（不属于永久红线）

- [ ] **P2-X01** 第二期完成静态和条件式补刀、连招、技能、走位与团战站位指导；基于完整战斗画面的逐时刻微操排入第三期；
- [ ] **P2-X02** 不把预测路线、概率位置或到达时间范围表达为确定事实，也不输出超出模型分辨率的伪精确坐标；
- [ ] **P2-X03** 第二期必须实现允许接口与屏幕可见行为支持的召唤师技能、大招和普通技能冷却估计；无法证明来源时显示未知并由外部 capability 控制；
- [ ] **P2-X04** 第二期实现用户触发和获准接口下的 Ping/聊天/喊话能力；不通过输入注入替玩家执行移动、施法、购买或点击；
- [ ] **P2-X05** 不接入 LLM，不做自由对话、持续麦克风或唤醒词；
- [ ] **P2-X06** 不让历史胜率、段位或战绩压过当前证据；
- [ ] **P2-X07** 不在没有空间证据时生成当前空间结论；
- [ ] **P2-X08** 其他地图和模式完成 discovery、数据集与适配器，正式支持排入第三期；
...
- [ ] **P3-188** 本地规则提供完整候选集合和排序信号；LLM 可以突出当前首选并解释备选，也可以拒答，但不能生成候选集合外动作或自动执行；
...
### 9.3 语音输入、持续收音与 ASR（必做）

离线与真实对局 PTT 都必须开发并测试；文本输入始终可用。麦克风与音频数据授权、地区和审核状态只决定外部版本是否显示对应开关。

- [ ] **P3-001** 默认使用 Push-to-Talk，同时必须提供用户主动开启的唤醒词和常开麦克风模式；三种模式分别授权、分别显示状态并可立即关闭；
- [ ] **P3-002** 支持按住说话、按下切换、唤醒词和持续监听；PTT/切换单次问句受 20 秒上限约束，持续模式按 VAD 切成独立短片段处理，不将整段音频无限缓冲或上传；
...
- [ ] **P3-008** 用户说话时可打断教练语音；
- [ ] **P3-009** 支持在来源可合法取得且用户逐项授权时接入系统 loopback、队伍语音或其他应用音频；每个来源独立显示采集中状态、本地优先、可立即撤回，未授权来源保持零采集；
...
- [ ] **P3-013** PTT/切换模式的单次问句硬上限 20 秒，连续静音 2.5 秒由 VAD 自动结束，15 秒时给出视觉预警；持续模式使用相同片段上限并自动滚动，不中断明确授权的监听状态；
- [ ] **P3-014** 到达硬上限、设备切换、权限撤回、应用退出或会话结束时立即停止采集并清理未提交缓冲；
- [ ] **P3-015** TTS 播放不能自动开启 ASR；实现回声抑制并将声学回灌纳入误触发评估；
- [ ] **P3-016** 采集只能由明确的快捷键、已启用的唤醒词或用户可见的持续监听状态机开始；异常按键抬起、设备状态或进程状态由 watchdog 及时结束对应片段；
- [ ] **P3-017** 记录单次时长、VAD 结束、超时、取消和上传字节数，但不在未授权遥测中记录音频内容。
- [ ] **P3-018** 队伍语音、系统声音、情绪和沟通分析按数据来源分别授权；界面持续显示正在分析的来源，支持单来源暂停、撤回和删除；
- [ ] **P3-019** 对他人语音只生成本局必要的沟通状态和战术摘要，不默认保留原始音频或建立身份画像；需要身份识别或长期留存时必须取得相关人员的明确授权。
...
- [ ] **P3-027** “重复上一条/用更短的话说”；
- [ ] **P3-028** “本局训练目标完成得怎么样？”；
- [ ] **P3-029** 支持英雄联盟、训练、赛事和游戏知识通用聊天；完全无关的开放域闲聊不作为三期验收阻塞项；
...
- [ ] **P3-045** 对“现在怎么办”只使用仍然有效的当前观测、迷雾预测、装备方案、计时、战斗评估、完整画面证据、授权沟通摘要与候选选项；
- [ ] **P3-046** 回答中每个当前观测和预测都能反查到证据，并明确区分 `observed` 与 `predicted`；
...
```text
PTT / 唤醒词 / 常开 VAD 片段 / 文字输入
  -> ASR final
  -> 意图分类与安全边界
  -> 冻结已验证上下文
  -> 确定性规则生成候选动作
  -> 本地排序首选与备选
  -> 受约束 LLM 表达首选、备选与适用条件，或拒答
  -> Schema 校验
  -> 事实引用与禁用内容校验
  -> 时效复核
  -> TTS
...
- [ ] **P3-051** 候选动作由本地规则提供；模型可以按证据和置信度排序并突出最高推荐，同时保留仍适用的备选条件，不得创造额外动作或自动执行；
- [ ] **P3-052** 当前观测、迷雾预测、装备建议、战斗胜算、完整画面结论和沟通摘要必须使用证据引用；缺少引用的句子删除或整条拒绝；
- [ ] **P3-053** 增加禁用自动操作、强制命令、无证据迷雾结论、保证性结论和攻击性内容校验；允许表达已经由本地模型验证并带置信度的迷雾预测；
...
- [ ] **P3-124 [必做]** 创作者、职业选手、公众人物或其他真人音色必须有权利人明确授权、用途/期限、AI 音色标识和撤销机制；
- [ ] **P3-125** 提供授权材料校验和到期停用；未取得权利人授权时不得模仿或克隆对应真人音色；
...
### 9.13 第三期授权与执行边界（功能必须开发，不属于永久红线）

- [ ] **P3-X01 [必做]** 提供用户主动选择的唤醒词与常开麦克风模式；队伍语音、系统声音和情绪/沟通分析使用独立授权、持续指示、本地优先和立即删除控制；
- [ ] **P3-X02 [必做]** 提供与英雄联盟、训练、赛事和游戏知识相关的通用对话；完全无关的开放域闲聊不作为验收阻塞项；
- [ ] **P3-X03 [必做]** 提供完整画面多模态分析；原始画面必须经用户授权、最小化裁剪和本地证据校验，未经验证的模型判断不能直接播音；
- [ ] **P3-X04** 不让 LLM 绕过本地候选动作、证据和时效校验；
- [ ] **P3-X05 [必做]** 提供主动、连续和可配置频率的 shot calling，包括最高置信推荐与备选方案；所有结论保留证据、置信度和撤销机制；
- [ ] **P3-X06** 不自动执行任何选择；
- [ ] **P3-X07 [必做]** 提供本人或已取得权利人明确授权的音色克隆；未授权真人音色仍属于永久红线；
- [ ] **P3-X08** 允许展示经过统计验证的个人训练效果、段位变化与胜率相关性，但营销文案不得承诺确定结果；
- [ ] **P3-X09 [必做]** 在端点、地区、授权和数据来源均合法时提供跨地区玩家实时/历史增强；不可用时明确降级；
- [ ] **P3-X10 [必做]** 加入 ARAM、斗魂竞技场、轮换模式、观战和微操教练，每种模式使用独立数据集、规则、能力开关和审核记录。
...
| 语音输入质量  | PTT 首尾截断率 <1%；唤醒词误唤醒/漏唤醒和常开分片丢失率达到预注册门槛；TTS 回灌导致的误触发为 0                            |
| 授权音频      | 未授权来源采集为 0；来源状态显示、单项撤回、片段清理和他人语音最小化测试 100% 通过                                         |
...
- [ ] `coach-voice-main`、PTT、唤醒词、常开 VAD 分片、授权音频、ASR、打断和多设备管理；
- [ ] 完整画面多模态、战斗胜算、逐帧微操、主动 shot calling 与首选/备选排序；
- [ ] ARAM、斗魂竞技场、轮换模式和观战教练的独立数据集、规则与能力开关；
...
| 音频设备     | 输出设备                | 延续                     | 增加输入设备、来源、增益与分项授权 |
| 快捷键       | 静音/暂停/重新标定      | 增加展开证据/反馈        | 增加 PTT/常开切换/打断/重复        |
...
- [ ] M11：PTT、唤醒词、常开麦、授权音频、ASR、打断与本地回退；
- [ ] M12：完整画面多模态、战斗胜算、逐帧微操、主动 shot calling、受约束 LLM 与事实校验；
- [ ] M13：知识库、玩家画像、训练闭环、排行榜、ARAM/Arena/轮换模式与观战教练；
- [ ] M14：隐私中心、云端预算、灰度与回滚；
- [ ] M15：内部完整能力验收；Gate 覆盖后按能力小流量外部上线并长期评估。
...
1. **第一期先把实时底座做可靠**：统一实时数据，完成小地图直接观测、迷雾概率推断、装备购买指导、基础连招/对线/计时/沟通提示、短语音和可回放评估。
2. **第二期把事实组合成更完整的宏观与条件式微操建议**：每条建议有证据、时效、首选与备选，并形成赛后关键时刻复盘。
3. **第三期加入完整对话和主动 AI 指挥**：覆盖 PTT、唤醒词、常开麦、经授权音频、完整画面多模态、逐帧微操、主动 shot calling、长期训练、其他模式与云端能力；技术故障可回退第二期，政策/反作弊/隐私外部启用故障则关闭外部实时开关，同时保留内部回归与修复能力。

不能反过来先接一个会说话的 LLM，再补视觉和证据。这个项目的核心竞争力不是“AI 说得像人”，而是“它能把直接观测、明确状态和标注过不确定性的推断统一成可追溯、及时且可撤销的教练建议”。
